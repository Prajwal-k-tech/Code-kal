...
export async function validateEmailDomain(email: string): Promise<ValidationResult> {
  ...
  const localMatch = domainsConfig.universities.find(uni => domain.endsWith(uni.domain));
  ...
  const matchesPattern = domainsConfig.patterns.some(pattern => domain.endsWith(pattern));
