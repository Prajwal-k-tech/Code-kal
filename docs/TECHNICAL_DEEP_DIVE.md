...
- ❌ Uploading student IDs  
- ❌ Trusting centralized databases
- ❌ Re-verifying at every merchant
...
```
Traditional:  "I'm alice@mit.edu" → merchant stores your email forever
ZeroKlue:     "I can prove I'm from a .edu domain" → merchant learns nothing else
```
...
```
┌────────────────────────────────────────────────────────────────────────────┐
│                           ZEROKLUE VERIFICATION FLOW                        │
├────────────────────────────────────────────────────────────────────────────┤
│                                                                            │
│  STEP 1: CONNECT WALLET                                                    │
│  ───────────────────────                                                   │
│  ┌──────────┐    ┌─────────────┐    ┌──────────────┐                      │
│  │  User    │───>│ RainbowKit  │───>│   wagmi      │                      │
│  │  clicks  │    │   modal     │    │   connect    │                      │
│  └──────────┘    └─────────────┘    └──────────────┘                      │
│                                            │                               │
│                                            ▼                               │
│                                     wallet address                         │
...
│  ┌──────────┐    ┌─────────────┐    ┌──────────────┐                      │
│  │  User    │───>│  Google     │───>│   Browser    │                      │
│  │  clicks  │    │  popup      │    │  receives    │                      │
│  │ "Verify" │    │  @uni.edu   │    │  id_token    │                      │
│  └──────────┘    └─────────────┘    └──────────────┘                      │
│                                            │                               │
│                                            ▼                               │
│                        JWT: { email, hd, exp, ... } + RSA signature        │
│                                                                            │
│  STEP 3: GENERATE EPHEMERAL KEY                                            │
│  ──────────────────────────────                                            │
│  ┌──────────────────────────────────────────────────┐                     │
│  │  Browser generates ed25519 keypair:              │                     │
│  │  - ephemeral_pubkey  (goes into JWT nonce)       │                     │
│  │  - ephemeral_secret  (never leaves browser)      │                     │
│  │  - expiry timestamp  (prevents replay)           │                     │
│  └──────────────────────────────────────────────────┘                     │
│                           │                                                │
│                           ▼                                                │
│  STEP 4: ZK PROOF GENERATION (~30 seconds)                                │
│  ─────────────────────────────────────────                                │
│  ┌────────────────────────────────────────────────────────────────┐       │
│  │  NoirJS + Barretenberg WASM                                    │       │
│  │                                                                │       │
│  │  PRIVATE INPUTS (hidden):                                      │       │
│  │  ├── Full JWT payload (email, name, etc.)                      │       │
│  │  └── JWT signature from Google                                 │       │
│  │                                                                │       │
│  │  CIRCUIT OPERATIONS:                                           │       │
│  │  ├── Parse JWT header.payload.signature                        │       │
│  │  ├── Verify RSA-SHA256 signature against Google's public key   │       │
│  │  ├── Extract email domain ("hd" claim)                         │       │
│  │  ├── Verify ephemeral key commitment matches JWT nonce         │       │
│  │  └── Derive ephemeral_pubkey as public output                  │       │
│  │                                                                │       │
│  │  PUBLIC OUTPUTS (85 total):                                    │       │
│  │  ├── jwt_pubkey_modulus_limbs[0..17]  (Google's RSA key)       │       │
│  │  ├── domain.storage[18..81]           (hashed domain)          │       │
│  │  ├── domain.len[82]                   (domain length)          │       │
│  │  ├── ephemeral_pubkey[83]             (unique session key)     │       │
│  │  └── ephemeral_pubkey_expiry[84]      (prevents replay)        │       │
│  └────────────────────────────────────────────────────────────────┘       │
│                           │                                                │
│                           ▼                                                │
│                   proof (Uint8Array) + publicInputs (bytes32[85])          │
│                                                                            │
│  STEP 5: SUBMIT TO SMART CONTRACT                                          │
│  ────────────────────────────────                                          │
│  ┌──────────────────────────────────────────────────────────────────┐     │
│  │  wagmi writeContract("verifyAndMint", proof, publicInputs)       │     │
│  │                           │                                      │     │
│  │                           ▼                                      │     │
│  │  ┌──────────────────────────────────────────────────────────┐   │     │
│  │  │  ZeroKlue.sol                                            │   │     │
│  │  │  ├── Verify publicInputs.length == 85                    │   │     │
│  │  │  ├── Extract ephemeralPubkey from publicInputs[83]       │   │     │
│  │  │  ├── Check !usedEphemeralKeys[ephemeralPubkey]           │   │     │
│  │  │  ├── Call HonkVerifier.verify(proof, publicInputs)       │   │     │
│  │  │  │       └── Returns true if proof is valid              │   │     │
│  │  │  ├── Store verification with timestamp                   │   │     │
│  │  │  ├── Mark ephemeralPubkey as used                        │   │     │
│  │  │  └── Emit StudentVerified event                          │   │     │
│  │  └──────────────────────────────────────────────────────────┘   │     │
│  └──────────────────────────────────────────────────────────────────┘     │
│                                                                            │
│  STEP 6: SUCCESS!                                                          │
...
│  │  └── Can prove status to any merchant              │                    │
│  └───────────────────────────────────────────────────┘                    │
│                                                                            │
└────────────────────────────────────────────────────────────────────────────┘
```
...
│                                                                            │
│  ┌──────────┐    ┌─────────────┐    ┌──────────────────────────┐          │
│  │  User    │───>│  Merchant   │───>│  ZeroKlue.isVerified()   │          │
│  │ connects │    │   site      │    │  or isRecentlyVerified() │          │
│  │  wallet  │    │   checks    │    │                          │          │
│  └──────────┘    └─────────────┘    └────────────┬─────────────┘          │
│                                                   │                        │
│                                     ┌─────────────┴─────────────┐         │
...
│  └── Anything except: "this wallet is verified"                            │
│                                                                            │
└────────────────────────────────────────────────────────────────────────────┘
```
...
- ❌ Show them the colors (they can't see the difference)
- ❌ Explain RGB values (too technical)
- ✅ ZK proof: Let them hide the balls behind their back, swap or not swap randomly, show you again. You always know if they swapped. After 20 rounds, the probability you're guessing is 1 in a million.
...
**Traditional Proof**: Show the JWT → merchant sees your email, name, everything
...
1. **Circuit Compilation**: Noir code → arithmetic circuit (gates)
...
│                                                                          │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │  HonkVerifier.sol (~1900 lines, auto-generated)                 │    │
│  │  ──────────────────────────────────────────────                 │    │
│  │  • Generated by: `nargo codegen-verifier`                       │    │
│  │  • Verification key embedded (from circuit)                     │    │
...
│                              ▼                                           │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │  ZeroKlue.sol (~200 lines)                                      │    │
│  │  ─────────────────────────                                      │    │
│  │                                                                 │    │
│  │  STATE:                                                         │    │
│  │  ├── mapping(address => StudentVerification) verifications      │    │
│  │  ├── mapping(bytes32 => bool) usedEphemeralKeys                 │    │
│  │  ├── IVerifier public immutable verifier                        │    │
│  │  └── uint256 public totalVerified                               │    │
│  │                                                                 │    │
│  │  CORE FUNCTIONS:                                                │    │
│  │  ├── verifyAndMint(proof, publicInputs)                         │    │
│  │  │   └── Verify proof + mint soulbound NFT                      │    │
│  │  ├── isVerified(address) → bool                                 │    │
│  │  │   └── Check if address has valid verification                │    │
│  │  ├── isRecentlyVerified(address, maxAge) → bool                 │    │
│  │  │   └── Check if verified within time window                   │    │
│  │  └── getVerification(address) → (verifiedAt, ephemeralPubkey)   │    │
│  │                                                                 │    │
│  │  SYBIL RESISTANCE:                                              │    │
│  │  • ephemeral_pubkey is marked as "used" after first mint        │    │
│  │  • Same ephemeral key cannot mint twice                         │    │
│  │  • User CAN re-verify with new ephemeral key (privacy rotation) │    │
│  │                                                                 │    │
│  │  NFT-LIKE FEATURES:                                             │    │
│  │  ├── balanceOf(address) → returns 1 if verified, 0 otherwise    │    │
│  │  └── transferFrom() → REVERTS (soulbound, cannot transfer)      │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│                                                                          │
└──────────────────────────────────────────────────────────────────────────┘
```
...
├── lib/
│   ├── providers/google-oauth.ts   # Google OAuth flow + JWT extraction
│   ├── circuits/jwt.ts             # Proof generation with NoirJS
│   ├── ephemeral-key.ts            # ed25519 key generation
│   ├── lazy-modules.ts             # Async WASM loading
│   ├── types.ts                    # TypeScript interfaces
│   └── utils.ts                    # Helper functions
├── hooks/
│   └── useStudentVerification.ts   # Main React hook
├── public/circuits/
...
**Now you know exactly what's happening under the hood! 🔬**
